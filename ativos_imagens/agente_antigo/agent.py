...
try:
    ...
    print("Aviso: Ferramenta de Áudio não disponível. Instale pydub: pip install pydub")
...
- 🎵 Efeitos sonoros MP3 (SFX)
- 🎬 Animações Lottie programáticas (LOAD, FBK, ACH)
- 🎨 Padrões e ícones SVG (UI, ICO)
- 🎞️ Animações do mascote (MAS-ANI)
- 🖼️ Molduras de badges (ACH)"""
        ...
        if asset_type == 'lottie_programmatic':
            ...
            result = _create_audio_asset(asset_id, spec, manager)
        else:
...
def _create_audio_asset(asset_id: str, spec: dict, manager: AssetManager) -> str:
    """Cria um ativo de áudio baseado nas especificações."""
    if not audio_available:
        return "❌ Erro: Ferramenta de Áudio não disponível. Instale com: pip install pydub requests"
    
    # Verificar limite de API antes de tentar
    if not check_api_limit('replicate'):
        return "❌ Limite de chamadas de API atingido. Tente novamente mais tarde."
    ...
    try:
        ...
        # Gerar o áudio (a classe já tem as configurações para cada ID)
...
✨ **Status do Checklist:** Atualizado para ✅ Criado pelo agente"""
        ...
        return f"❌ Erro ao gerar áudio para '{asset_id}': {str(e)}"
...
"""
        ...
        # Listar Áudio
        if creatable.get('audio'):
            report += f"\n🎵 **Efeitos Sonoros MP3 ({len(creatable['audio'])}):**\n"
            for asset_id in sorted(creatable['audio']):
        ...
        # Listar Animações do Mascote
        if creatable.get('lottie_mascote'):
            report += f"\n🎞️ **Animações do Mascote ({len(creatable['lottie_mascote'])}):**\n"
            for asset_id in sorted(creatable['lottie_mascote']):
                spec = manager.asset_specs.get(asset_id, {})
                status = "✅" if manager.checklist_status.get(asset_id, {}).get('completed') else "⏳"
                report += f"  {status} {asset_id}: {spec.get('filename', 'N/A')}\n"
...
- `create_asset(asset_id)`: Cria um ativo específico usando seu ID (ex: 'SFX-01', 'LOAD-01', 'ICO-02')
- `check_asset_inventory()`: Lista todos os ativos disponíveis e seus status
- `get_project_status()`: Mostra o status geral do projeto

Você pode criar os seguintes tipos de ativos:
• 🎵 **Áudio MP3** (SFX): 9 efeitos sonoros infantis usando IA (stable-audio/musicgen)
• 🎬 **Animações Lottie** (LOAD, FBK, ACH): Animações programáticas e do mascote
• 🎨 **Vetores SVG** (UI, ICO): Ícones e padrões usando Recraft AI
• 🖼️ **Imagens PNG**: Mascote PROF e elementos visuais usando Replicate

Quando o usuário perguntar sobre suas capacidades, sempre use `check_asset_inventory()` para mostrar a lista atualizada e precisa de ativos que pode criar.
""",
        tools=[
